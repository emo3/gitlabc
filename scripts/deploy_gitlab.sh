...
# Helm reconciliation is repeat-safe; restart consumers only after intentionally
# changing external dependency secrets.
RESTART_GITLAB_WORKLOADS="${RESTART_GITLAB_WORKLOADS:-false}"
...
GITLAB_CHART_VERSION="${GITLAB_CHART_VERSION:-10.2.0}"
...
case "${GITLAB_DEPLOY_PROFILE}" in
  local|local-mkcert)
    ...
    PROFILE_SET_ARGS=(
      ...
      --set "global.hosts.ssh=${GITLAB_SSH_HOST}"
      --set "global.shell.port=${GITLAB_SHELL_SSH_PORT}"
